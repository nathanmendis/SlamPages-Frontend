...
const SlamSubmissionPage = () => {
  ...
  const fetchBookDetails = async () => {
    try {
      ...
      setTheme(response.data.theme); // default to book theme
  ...
  return (
    <div className="min-h-screen py-16 px-6 max-w-3xl mx-auto">
      ...
      {/* Style selector for entry page */}
      <div className="bg-stone-100 border-x border-stone-200 p-4 flex flex-col sm:flex-row justify-between items-center gap-4 font-sans text-xs">
        <span className="font-bold text-stone-600">Theme your entry sheet:</span>
        <div className="flex flex-wrap gap-2">
          {['School Notebook', 'Y2K Cyber', 'Polaroid Memory', 'Vintage Diary', 'Dark Academia'].map(th => (
            <button
              key={th}
              type="button"
              onClick={() => setTheme(th)}
              className={`px-3 py-1.5 rounded-lg border font-bold transition ${
                theme === th ? 'bg-amber-900 border-amber-950 text-white' : 'bg-white border-stone-300 hover:bg-stone-50 text-stone-700'
              }`}
            >
              {th}
            </button>
